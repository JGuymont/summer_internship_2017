...
p = parSapply(cl, 1:num_sim, function(i) {
  ...
  quantity = c(runif(1,min=-0.1,max=0.1),runif(1,min =-0.1,max=0.1),runif(1,min=-0.1,max=0.1),runif(1,min=-0.1,max=0.1))
...
for (i in 1:num_sim){
  ...
  hyld[i] = detrend(yld + W1 + W2 + W3 + W4, annual_data$t, TRUE)[test_year] 
}
